            ...
            "properties": {
                ...
                "text": {
                    ...
                    "description": (
                        ...
                        "Examples: 'car', 'person', 'apple', 'bird'."
                    ),
                },
            },
    ...
    def call(
        ...
        try:
            ...
            if not raw.get("success"):
                return {
                    "success": False,
                    "error": raw.get("error", "CountGD counting failed"),
                }

            boxes = raw.get("boxes", []) or []
            count = int(raw.get("count", len(boxes)) or 0)
            ...
            description = raw.get("description") or (
                f"CountGD counted {count} '{text}' object(s) in the image."
            )
            extras = {k: v for k, v in raw.items() if k not in _ENVELOPE_KEYS}
            extras.setdefault(
                "result",
                {"count": count, "boxes": boxes, "text": text},
            )
            extras["count"] = count
            return ToolResult(
                success=True,
                ...
                vis_path=raw.get("output_path") or raw.get("vis_path"),
                **extras,
            )
